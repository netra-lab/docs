...
The acrued reward for each FMA is calculated using [cumulative function](#cummulative-function) that is based by the [base point](#base-point), which is influenced by the **fiat price of an FMA**. The **distribution will happen hourly** based on the duration of when and how long the FMA has been held after minting or last claim.
